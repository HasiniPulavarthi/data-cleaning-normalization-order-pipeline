# Case Study: Cleaning, Deduplicating, and Normalizing a Denormalized Order Export

**Scope:** a synthetic-but-realistic 728-row flat CSV export (the kind you'd get from
...
All numbers below are pulled directly from running the pipeline — not estimated.

---

## 1. The problem

The source file, `messy_orders.csv`, is a single flat table: every order line
...
| File | Purpose |
|---|---|
| `generate_messy_data.py` | Generates the synthetic messy source data (documents exactly what issues were injected) |
| `messy_orders.csv` | The raw, denormalized, 728-row input |
...
| `run_pipeline.py` | Registers the phone/date UDFs, runs all three SQL scripts in order, prints before/after metrics |
| `cleaning_project.db` | Resulting SQLite database (raw + staging + final tables, queryable directly) |
| `clean_customers.csv`, `clean_products.csv`, `clean_orders.csv`, `clean_order_items.csv` | Final normalized tables exported flat, for anyone who wants the result without SQLite |

To reproduce end to end: `python3 generate_messy_data.py && python3 run_pipeline.py`.
